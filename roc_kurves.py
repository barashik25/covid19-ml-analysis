...
# Загружаем данные и обучаем модели (повторяем из model_training.py)
...
# Строим ROC-кривые — два графика рядом
...
ax.set_title("ROC-кривые — Датасет 1 (JRC)", fontsize=13)
...
ax.set_xlabel("False Positive Rate", fontsize=12)
ax.set_ylabel("True Positive Rate", fontsize=12)
ax.set_title("ROC-кривые — Датасет 2 (GDSI/MS)", fontsize=13)
ax.legend(loc="lower right", fontsize=10)
ax.grid(alpha=0.3)

plt.suptitle("Сравнение ROC-кривых: Random Forest vs MLP", fontsize=14, fontweight="bold")
plt.tight_layout()
